...
# dateRangeInputOriginal("datesOriginal",
#                 "Filter observations/complaints by date:",
...
filteredComplaints <- reactive({ nrow(filterAirnoiseComplaints()) })

output$dataSelectedStatisticsText <- renderText({
  str_glue(
    "This filter results in a total of {filteredComplaints()} ",
    "observations, during {dayCount} days.",
    "The result is an average of {avgReports} reports per day.",
    dayCount = dataSelectedInterval() / days(1),
    avgReports = round(filteredComplaints() / dayCount, digits = 1)
  )
})
